...
class CyberSeedsSnapshot extends HTMLElement {
  ...
  close(){
    // Confirm only if mid-run (not intro / not complete)
  ...
  /* ---------------- Seed selection (deterministic, local-first) ---------------- */
  ...
  pickSeedPackForLens(lens, snapshotId){
    // If your seeds.json doesn’t exist or has no matches, we fallback calmly.
    ...
    // Your seed objects might use different field names — we normalize lightly.
  ...
  canonicalize(scored){
    ...
    // Rationale from engine (if available)
    let rationale = "";
    try{
    ...
    // Deterministic seed pack (local only)
  ...
  finish(){
    try{
      ...
      // ✅ Keep compatibility with your current homepage listener: e.detail.snapshot
